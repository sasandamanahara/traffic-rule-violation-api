...
#     if not helmet_model or not triple_riding_model or not model:
#         return jsonify({
#             'error': 'Models not loaded. Please ensure model files exist in the models/ directory.',
#             'details': {
#                 'helmet_model': helmet_model is not None,
#                 'triple_riding_model': triple_riding_model is not None,
#                 'rider_model': model is not None
#             }
#         }), 500
...
#         if frame_count == 0:
#             cap.release()
#             return jsonify({'error': 'Invalid video file or unable to read frames'}), 400
...
#             except Exception as e:
#                 print(f"Error in helmet model tracking: {e}")
#                 results_helmet = None
                
#             try:
#                 results_tripleriding = triple_riding_model.track(frame, persist=True, conf=0.4, tracker="botsort.yaml")
#             except Exception as e:
#                 print(f"Error in triple riding model tracking: {e}")
#                 results_tripleriding = None

#             # YOLOv5 rider detection
#             try:
#                 results_rider = model(frame)
#             except Exception as e:
#                 print(f"Error in rider model: {e}")
#                 results_rider = None
                
#             conf_thresh = 0.3
#             rider_boxes = []
#             if results_rider and hasattr(results_rider, 'xyxy') and len(results_rider.xyxy) > 0 and results_rider.xyxy[0].shape[0] > 0:
...
#                 helmet_count = len(results_helmet[0].boxes) if results_helmet and len(results_helmet) > 0 and results_helmet[0].boxes is not None else 0
#                 triple_count = len(results_tripleriding[0].boxes) if results_tripleriding and len(results_tripleriding) > 0 and results_tripleriding[0].boxes is not None else 0
#                 rider_count = len(rider_boxes)
#                 print(f"Frame {current_frame_idx}: Riders={rider_count}, Helmets={helmet_count}, Triple={triple_count}")

#             # Collect helmet boxes with fill_ratio
#             # Note: Helmet model likely detects "no helmet" violations (class 1) or "helmet" (class 0)
#             # We want to detect violations, so we check all detections
#             helmet_boxes = []
#             if results_helmet and len(results_helmet) > 0 and results_helmet[0].boxes is not None:
#                 try:
...
#                             # Process all helmet detections (both helmet and no-helmet)
#                             # If riders are detected, check if helmet belongs to rider
#                             if len(rider_boxes) > 0:
#                                 belongs, fill_ratio = box_belongs_to_rider(box, rider_boxes, threshold=0.7)  # Lowered threshold
#                                 if belongs or conf > 0.5:  # If high confidence, record it
#                                     helmet_boxes.append((box, fill_ratio if belongs else 0.5))
#                             else:
#                                 # If no riders detected but helmet model detected something, still record it
#                                 # (might be detecting no-helmet violations directly)
#                                 if conf > 0.3:  # Lower confidence threshold when no riders
#                                     helmet_boxes.append((box, 0.0))
#                 except Exception as e:
#                     print(f"Error processing helmet boxes: {e}")

#             # Triple riding boxes with fill_ratio
#             triple_boxes = []
#             if results_tripleriding and len(results_tripleriding) > 0 and results_tripleriding[0].boxes is not None:
#                 try:
#                     for box in results_tripleriding[0].boxes:
#                         conf = float(box.conf.cpu().numpy()[0]) if box.conf is not None else 0.0
#                         if len(rider_boxes) > 0:
#                             belongs, fill_ratio = box_belongs_to_rider(box, rider_boxes, threshold=0.7)  # Lowered threshold
#                             if belongs or conf > 0.4:  # Triple riding model has conf=0.4 threshold
#                                 triple_boxes.append((box, fill_ratio if belongs else 0.5))
#                         else:
#                             # If no riders detected but triple riding detected, it's still a violation
#                             if conf > 0.3:  # Lower confidence threshold when no riders
#                                 triple_boxes.append((box, 0.0))
#                 except Exception as e:
#                     print(f"Error processing triple riding boxes: {e}")
...
#                 try:
#                     # Get track ID - use frame number if no tracking ID available
#                     if hasattr(box_obj, 'id') and box_obj.id is not None:
#                         try:
#                             track_id = int(box_obj.id.cpu().numpy()[0])
#                         except:
#                             track_id = current_frame_idx  # Use frame number as fallback
#                     else:
#                         track_id = current_frame_idx  # Use frame number as fallback
                    
#                     # Create unique key - use frame number to allow multiple violations per track
#                     key = (model_name, track_id, current_frame_idx)

#                     if model_name != 'Rider' and key not in violation_ids:
#                         violation_ids.add(key)

#                         if box_obj.xyxy is not None and len(box_obj.xyxy) > 0:
#                             xyxy = box_obj.xyxy.cpu().numpy().astype(int)[0].tolist()
#                             confidence = float(box_obj.conf.cpu().numpy()[0]) if box_obj.conf is not None else 0.0
...
#                             })
#                 except Exception as e:
#                     print(f"Error processing violation for {model_name}: {e}")
#                     continue
...
#                 except Exception as e:
#                     print(f"Error drawing box for {model_name}: {e}")
#                     continue
...
#         print(f"   Violations Detected: {len(violation_ids)}")
#         print(f"   Violations Data: {len(violations_data)}")
#         print(f"   Saved to DB: {len(saved_violation_ids)}")
