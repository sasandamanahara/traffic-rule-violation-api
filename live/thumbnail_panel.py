...
class ThumbnailPanel:
    ...
    def __init__(self):
        ...
        self.canvas = tk.Canvas(self.thumb_frame, width=250, height=700, bd=0, highlightthickness=0)
        ...
        # Anchor top-center so it can be moved horizontally
    ...
    def update_video(self, frame):
        try:
            ...
            img = img.resize((1300, 950))  # Adjust as needed
    ...
    def add_thumbnail(self, img_array, vtype, obj_id):
        """Add a violation thumbnail to the scrollable frame with debug colors."""
        try:
        ...
        block = ttk.Frame(self.frame, width=300, height=h_size+30)
        # DEBUG color for the block
        block.config(style=f"Block.TFrame")
        block.pack(pady=8, fill="x")  # fill horizontally
        block.columnconfigure(0, weight=1)  # allow centering inside block

        # Label for violation type + obj_id
        lbl_text = ttk.Label(block, text=f"{vtype}", font=("Arial", 11, "bold"),
                             background="#cccccc")
        lbl_text.pack(anchor="center")

        # Image label
        lbl_img = ttk.Label(block, image=tk_img, background="#aaaaaa")
...
if __name__ == "__main__":
    ...
    panel.root.after(100, generate_dummy_video)
    panel.run()
