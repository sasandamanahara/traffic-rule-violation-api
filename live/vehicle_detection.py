...
def detect_vehicles(video_source, calibration):
    ...
    while True:
        ...
        if results_vehicle and results_vehicle[0].boxes.id is not None:
            ...
            # --- Speed detection (as before, per vehicle) ---
            for box, obj_id, cls in zip(boxes, ids, classes):
                x1, y1, x2, y2 = map(int, box)
                cx, cy = int((x1 + x2)/2), int((y1 + y2)/2)

                # Calculate speed using original individual boxes
            ...
            # Separate motor and rider boxes
            motor_boxes = []
            motor_ids = []
            rider_boxes = []
            ...
            for i, motor_box in enumerate(motor_boxes):
                ...
                for rider_box in rider_boxes:
                    ...
                    if (x1 < rx2 and x2 > rx1 and y1 < ry2 and y2 > ry1):
                        # expand motor box to include rider
                        x1 = min(x1, rx1)
                        y1 = min(y1, ry1)
                        x2 = max(x2, rx2)
                        y2 = max(y2, ry2)

                # Use combined crop only for helmet/triple detection
