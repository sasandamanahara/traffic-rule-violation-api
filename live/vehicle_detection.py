...
class VehicleDetector:
    ...
    # ------------------------------
    # PROCESSING & DISPLAY THREAD
    # ------------------------------
    def processing_thread(self):
        while not self.stopped:
            try:
                frame, results_vehicle = self.detection_queue.get(timeout=1)
            except queue.Empty:
                continue
            ...
            for obj in tracked_objects:
                ...
                if (not obj["crossed"]
                    ...
                    cv2.imwrite(os.path.join(
                        VIOLATION_OUTPUT_DIR, f"violation_{self.frame_idx}_{vid_str}.jpg"), crop)
    ...
    # ------------------------------
    def run(self):
        threading.Thread(target=self.rtsp_reader, daemon=True).start()
